...
from rubiks.palette_class import Palette, PaletteWeights, CombinedPalette, CombinedPaletteWeights
from rubiks.transformations import (
    Transformation,
    NoneTransformation,
    RecolourClosest,
...
class RecolourClosestCombinedTest(unittest.TestCase):
    """
    Test the RecolourClosestCombined class.
    """
    ...
    def test_regular_no_weights(self) -> None:
        ...
        np.testing.assert_array_equal(RecolourClosestCombined.transform_image(image, self.palette), equivalent_image)
    ...
    def test_missing_weights(self) -> None:
        ...
        invalid_weights = PaletteWeights({"white": 1})
        with self.assertRaises(ValueError):
            RecolourClosestCombined.transform_image(self.image, self.palette, palette_weights=invalid_weights)
