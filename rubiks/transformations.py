...
class TransformationFromCombinedPalette(Transformation):
    """
    A transformation which takes a palette (and weights) as inputs, then creates the corresponding combined versions.
    """
...
class RecolourClosestCombined(TransformationFromCombinedPalette):
    """
    Recolour each pixel to the colour in the palette or combined palette that is closest geometrically.
    """
    ...
    def _transform_pixel(
        ...
        combinations of colours. If a combination of two colours is closest, one of those two colours will be chosen
        based on the position of the pixel within its image.
        ...
        return recolour_closest_combined(pixel, palette, palette_weights, combined_palette, combined_palette_weights)
